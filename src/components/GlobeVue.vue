<template>
	<audio id="player" volume="0.1" autoplay loop>
		<source src="/sound/Polozhenie.mp3" type="audio/mpeg">
...
export default {
	data () {
		return{
			...
			camera: null,
			renderer: null,
			localHours : (new Date).getHours() +":"+(new Date).getMinutes() +":"+(new Date).getSeconds() 
		}
	},
	mounted() {
		...
		// dat.gui controls
		guiCamera.add(cameraControls, 'speed', 0, 0.1).step(0.001).onChange(
			(value) => {
				cameraRotationSpeed = value;
			}
		);
		guiCamera.add(cameraControls, 'orbitControls').onChange(
			(value) => {
		...
		guiSurface.add(surfaceControls, 'rotation', 0, 6).onChange(
			(value) => {
		...
		guiSurface.add(surfaceControls, 'shininess', 0, 30).onChange(
			(value) => {
				earth.getObjectByName('surface').material.shininess = value;
			}
		);

		guiAtmosphere.add(atmosphereControls, 'opacity', 0, 1).onChange(
			(value) => {
		...
		guiAtmosphericGlow.add(atmosphericGlowControls, 'intensity', 0, 1).onChange(
			(value) => {
				earth.getObjectByName('atmosphericGlow').material.dat['c'].value = value;
			}
		);
